...
//       from: "Stall/Booth Request <noreply@differentstrokes.in>",
//       // to: "connect@differentstrokes.in,venkat@differentstrokes.in", // Admin email
//       to: "dhavalpatil2345@gmail.com",
//       subject: `New Stall Request from ${formData.companyName}`,
  ...
  try {
    ...
    if (formData.companyLogo) {
      try {
        ...
        const base64Data = formData.companyLogo.replace(/^data:image\/[a-z]+;base64,/, '')
    ...
    if (formData.floorPlan) {
      try {
        ...
        const base64Data = formData.floorPlan.replace(/^data:application\/pdf;base64,/, '')
    ...
    const adminEmailPayload = {
      from: "Stall/Booth Request <noreply@differentstrokes.in>", // Added proper from address
      to: "connect@differentstrokes.in, venkat@differentstrokes.in", // Replace with actual admin email
      subject: `New Stall Request from ${formData.companyName}`,
      html: generateAdminEmailHtml(formData),
    }
    ...
    const { data: adminData, error: adminError } = await resend.emails.send(adminEmailPayload)
...
<body style="font-family: Arial, sans-serif; margin: 0; padding: 20px; background-color: #f8f8f8;">
  ...
  <table>
    ...
    <tr>
      <td>Company Logo</td>
      <td>${formData.companyLogo ? "✅ Attached" : "❌ Not provided"}</td>
    </tr>
    <tr>
      <td>Branding Description</td>
      <td>${formData.brandingDescription || "N/A"}</td>
  ...
  <table>
    ...
    <tr>
      <td>Floor Plan</td>
      <td>${formData.floorPlan ? "✅ Attached" : "❌ Not provided"}</td>
    </tr>
  </table>

  <div style="margin-top: 30px; padding: 15px; background-color: #e8f4fd; border-left: 4px solid #4a90e2;">
    <p><strong>Note:</strong> Please check the attached files for company logo and floor plan if provided.</p>
  </div>
</body>
</html>
  `
